...
*
* @return str|array
* @contributor WPExplorer
*/

// aq_resize Renamed to ssp_resize for use with Symple Shortcodes Premium

if ( !function_exists( 'ssp_img_resize' ) ){
	function ssp_img_resize( $url, $width, $height = null, $crop = null, $single = true ) {
	...
	if ( function_exists('wpex_get_data') ) {
		if ( wpex_get_data( 'retina', '1' ) == '1' ) {
				...
				if(file_exists($destfilename) && getimagesize($destfilename)) {	
					...
					$retina_width = $aq_width*2;
					$retina_height = $aq_height*2;
					$editor = wp_get_image_editor($img_path);
					$editor->resize( $retina_width, $retina_height, $crop );
					$filename = $editor->generate_filename( $aq_width . 'x' . $aq_height . '@2x'  );
